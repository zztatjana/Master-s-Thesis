plotProbabilisticFeasibilitySpace <- function(data.row,
  ...
  # Plot Gompertz model instead of logistic if switch activated (needed for comparison with gompertz model)
  if (gompertz == T){
    data.plot <- data.plot %>% 
  ...
  data.plot.targets <- data.plot.targets %>%
    mutate(category = case_when(
      #Net Zero Scenarios
      region == "Europe" & year %in% c(2030, 2050) & unit == "tCO2" & value %in% c(5000000, 152000000) ~ "Net Zero Scenarios",
      region == "North America" & year %in% c(2025) & unit == "tCO2" & value %in% c(1000000) ~ "Net Zero Scenarios",
      region == "Global" & year %in% c(2030, 2050) & unit == "tCO2" & value %in% c(85000000, 980000000) ~ "Net Zero Scenarios",
      
      #IPCC Ratio Scenarios
      region == "Europe" & year %in% c(2025) & unit == "tCO2" & value %in% c(300000) ~ "IPCC Ratio Scenarios",
      region == "North America" & year %in% c(2030, 2050) & unit == "tCO2" & value %in% c(38380000, 549240000) ~ "IPCC Ratio Scenarios",
      region == "Global" & year %in% c(2025) & unit == "tCO2" & value %in% c(14170000) ~ "IPCC Ratio Scenarios",
  ...
  p.facet <- ggplot() +
    ...
    scale_fill_manual(
      name = NULL,
      values = c(
        "Net Zero Scenarios" = "magenta",
        "IPCC Ratio Scenarios" = "cyan"
      ),
    ...
    expand = expansion(mult = c(0.01, 0.05))  # Hier kann der Wert angepasst werden
    ) +                   
    # Vertical line at X
